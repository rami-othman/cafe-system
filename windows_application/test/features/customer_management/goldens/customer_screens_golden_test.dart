...
void main() {
  testWidgets('customer screens own every ready locale/viewport row', (
    tester,
  ) async {
    ...
    for (final _GoldenRow row in _rows) {
      ...
      await expectLater(
        ...
        matchesGoldenFile('baselines/customer_screens/${row.name}.png'),
      );
    }
  });
}
...
final List<_GoldenRow> _rows = <_GoldenRow>[
  ..._rowsFor('en_ltr', Locale('en'), TextDirection.ltr),
  ..._rowsFor('ar_rtl', Locale('ar'), TextDirection.rtl),
];
...
List<_GoldenRow> _rowsFor(
  ...
  final Map<String, _GoldenScreen> screens = <String, _GoldenScreen>{
    'customer_list': (fixtures) => fixtures.customerList(),
    'customer_detail': (fixtures) => fixtures.customerDetail(),
    'customer_create': (fixtures) => fixtures.customerCreate(),
    'customer_edit': (fixtures) => fixtures.customerEdit(),
  };
