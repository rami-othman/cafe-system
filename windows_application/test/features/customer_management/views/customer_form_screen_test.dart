...
void main() {
  testWidgets(
    'create form starts with a required phone and does not display a customer number',
    (tester) async {
      ...
      expect(find.byKey(const Key('customer-phone-0-raw')), findsOneWidget);
      ...
      expect(find.text('Phone numbers *', findRichText: true), findsOneWidget);
      expect(find.text('Customer groups'), findsOneWidget);
      expect(find.text('Notes'), findsWidgets);
    },
  );

  testWidgets('selected groups use a readable warm label color', (
    tester,
  ) async {
    await tester.pumpWidget(_host(_FormRepository()));
    await tester.pumpAndSettle();
    ...
    final FilterChip chip = tester.widget<FilterChip>(
      find.byKey(const Key('customer-group-chip-3')),
    );
    expect(chip.selected, isTrue);
    expect(chip.labelStyle?.color, const Color(0xFF6B4226));
  });
  ...
  testWidgets(
    'reference-aligned edit sections keep the number hint, lifecycle, and footer inside the form card',
    (tester) async {
      await tester.pumpWidget(_host(_FormRepository(), customerId: 7));
      await tester.pumpAndSettle();

      final Finder surface = find.byKey(const Key('customer-form-surface'));
      expect(
        find.byKey(const Key('customer-form-information-fields')),
        findsOneWidget,
      );
      expect(
        find.text('The customer number is generated by the server.'),
        findsOneWidget,
      );
      expect(
        find.descendant(
          of: surface,
          matching: find.byKey(const Key('customer-form-lifecycle-section')),
        ),
        findsOneWidget,
      );
      expect(
        find.descendant(
          of: surface,
          matching: find.byKey(const Key('customer-form-footer')),
        ),
        findsOneWidget,
      );
    },
