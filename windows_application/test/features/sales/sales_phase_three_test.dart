...
void main() {
  group('models', () {
    test('SalesInvoice parses derived paid/remaining/overdue/collections', () {
      final SalesInvoice invoice = SalesInvoice.fromJson(<String, dynamic>{
        'id': 10,
        'invoiceNumber': 'SI-2026-000010',
        'customerId': 2,
        'customerName': 'Damascus Tech',
        'branchId': 3,
        'branchName': 'Downtown',
    ...
    test(
      'CustomerReceivablesSummary orders open invoices as the server sent them (oldest-due-first)',
      () {
        final CustomerReceivablesSummary summary =
            CustomerReceivablesSummary.fromJson(<String, dynamic>{
  ...
  group('Sales Center list', () {
    testWidgets(
      'shows paid/remaining/payment-status columns and a Register Payment action',
      (tester) async {
        ...
        final SalesCubit cubit = SalesCubit(
          repository: SalesRepository(DioApiClient(dio: dio)),
        );
        await tester.pumpWidget(
          MaterialApp(
            home: Directionality(
              ...
              child: Scaffold(
                body: BlocProvider<SalesCubit>.value(
                  value: cubit,
        ...
        expect(find.text('100.00 SYP'), findsOneWidget);
        expect(find.text('200.00 SYP'), findsOneWidget);
  ...
  group('Sales Invoice detail', () {
    Widget buildDetail(Dio dio) {
      final SalesCubit salesCubit = SalesCubit(
        repository: SalesRepository(DioApiClient(dio: dio)),
      );
    ...
    testWidgets(
      'shows invoice total/paid/remaining/status and collection history, with Register Payment visible',
      (tester) async {
        await tester.binding.setSurfaceSize(const Size(1600, 900));
        addTearDown(() => tester.binding.setSurfaceSize(null));
        final Dio dio = Dio(BaseOptions(baseUrl: 'http://test.local/api/v1/'));
    ...
    testWidgets(
      'a draft invoice shows Post and Post-and-Collect but no Register Payment',
      (tester) async {
        await tester.binding.setSurfaceSize(const Size(1600, 900));
        addTearDown(() => tester.binding.setSurfaceSize(null));
        final Dio dio = Dio(BaseOptions(baseUrl: 'http://test.local/api/v1/'));
  ...
  group('CustomerPaymentDialog', () {
    testWidgets(
      'auto-allocates oldest-due-first and posts a balanced allocation',
      (tester) async {
        ...
        dio.interceptors.add(
          InterceptorsWrapper(
            onRequest: (options, handler) {
                    ...
                    data: <String, dynamic>{
                      'data': <Map<String, dynamic>>[
                        <String, dynamic>{
                          ...
                          'isActive': true,
                          'financialLocationId': 7,
                          'financialLocationName': 'الصندوق',
                        },
                      ],
                      'meta': <String, dynamic>{
                        'currentPage': 1,
              ...
              if (options.path == 'finance/customer-payments' &&
                  options.method == 'POST') {
                posted.add(options);
                return handler.resolve(
                  Response<dynamic>(
                ...
                body: MultiBlocProvider(
                  ...
                  child: Builder(
                    builder: (context) => ElevatedButton(
                      onPressed: () => CustomerPaymentDialog.show(
                        context,
                        salesRepository: salesCubit.repository,
                        financeSetupRepository: financeCubit.repository,
                        customerId: 2,
                        customerName: 'Damascus Tech',
                        branchId: 3,
                      ),
        ...
        expect(
          find.text('تسجيل دفعة من العميل — Damascus Tech'),
          findsOneWidget,
        );
        ...
        await tester.enterText(
          find.byKey(const Key('customerPaymentAmountField')),
          '150.00',
        );
        await tester.pumpAndSettle();
        ...
        expect(
          tester
              .widget<TextField>(find.byKey(const Key('allocationField-1')))
              .controller!
              .text,
          '50.00',
        );
        expect(
          tester
              .widget<TextField>(find.byKey(const Key('allocationField-2')))
              .controller!
              .text,
          '100.00',
        );
    ...
    testWidgets('blocks posting while any amount is left unallocated', (
        ...
        InterceptorsWrapper(
          onRequest: (options, handler) {
            ...
            return handler.reject(
              DioException(
                requestOptions: options,
                message: 'unexpected POST during unallocated-amount test',
              ),
            );
          },
        ),
      ...
      await tester.enterText(
        find.byKey(const Key('allocationField-1')),
        '100.00',
      );
      await tester.pumpAndSettle();
