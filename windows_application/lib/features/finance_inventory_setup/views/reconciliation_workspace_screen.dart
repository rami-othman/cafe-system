...
class ReconciliationWorkspaceScreen extends StatefulWidget {
  const ReconciliationWorkspaceScreen({
    super.key,
  ...
  @override
  State<ReconciliationWorkspaceScreen> createState() =>
      _ReconciliationWorkspaceScreenState();
}

class _ReconciliationWorkspaceScreenState
    extends State<ReconciliationWorkspaceScreen> {
  ReconciliationSession? _session;
  List<ReconciliationSystemTransaction> _transactions =
      const <ReconciliationSystemTransaction>[];
  List<ReconciliationSuggestion> _suggestions =
  ...
  Widget _buildBody() {
    if (_loading) {
      return const FinanceLoadingState(label: 'جارٍ تحميل التسوية…');
    }
    if (_error != null) {
      return FinanceErrorState(
        message: 'تعذّر تحميل التسوية. $_error',
        onRetry: _load,
      );
    }
    final ReconciliationSession? session = _session;
    if (session == null) {
      return const FinanceErrorState(message: 'تعذّر إيجاد التسوية المطلوبة.');
    }
        ...
        children: <Widget>[
          ...
          FinanceKpiGrid(
            items: <FinanceKpiData>[
              FinanceKpiData(
                label: 'الرصيد الافتتاحي (دفتر)',
                value: session.balances.bookOpening ?? '—',
              ),
              FinanceKpiData(
                label: 'الرصيد الختامي (دفتر)',
                value: session.balances.bookClosing ?? '—',
              ),
              FinanceKpiData(
                label: session.type == 'cash'
                    ? 'النقد الفعلي'
                    : 'الرصيد الفعلي/الكشف',
                value:
                    (session.type == 'cash'
                        ? session.balances.actualCash
                        : session.balances.externalClosing) ??
                    '—',
              ),
              FinanceKpiData(
                label: 'الفرق',
              ...
              FinanceKpiData(
                label: 'المطابق',
                value: session.summary.matchedAmount,
              ...
              FinanceKpiData(
                label: 'نسبة التقدم',
                value: '${session.summary.progressPercent.round()}%',
              ),
            ],
          ),
          const SizedBox(height: FinanceSpace.lg),
          ...
          if (session.type == 'cash')
            ...
            _BankCardWorkspace(
              ...
              onUpdateExternalBalance: () =>
                  _openBalanceDialog(actualCash: false),
            ),
          const SizedBox(height: FinanceSpace.lg),
          if (!completed)
            _OperationalBar(
              ...
              onUpdateCash: session.type == 'cash'
                  ? () => _openBalanceDialog(actualCash: true)
                  : null,
            ),
        ],
      ),
  ...
  void _toggleSystem(int journalEntryId) => setState(() {
    if (!_selectedSystem.remove(journalEntryId)) {
      _selectedSystem.add(journalEntryId);
    }
    _matchError = null;
  });
  void _toggleStatement(int lineId) => setState(() {
    if (!_selectedStatement.remove(lineId)) {
      _selectedStatement.add(lineId);
    }
    _matchError = null;
  });

  double _systemRemaining(ReconciliationSystemTransaction t) =>
      _amount(t.amount) - _amount(t.matchedAmount);

  Future<void> _performMatch() async {
    final ReconciliationSession? session = _session;
    if (session == null) return;
    final List<ReconciliationSystemTransaction> selectedSystem = _transactions
        .where(
          (ReconciliationSystemTransaction t) =>
              _selectedSystem.contains(t.journalEntryId),
        )
        .toList();
    final List<ReconciliationStatementLine> selectedStatement = session
        .statementLines
        .where(
          (ReconciliationStatementLine l) => _selectedStatement.contains(l.id),
        )
        .toList();
    ...
    if (selectedSystem.length > 1 && selectedStatement.length > 1) {
      setState(
        () => _matchError =
            'اختر عنصراً واحداً من أحد الجانبين عند مطابقة أكثر من عنصر بالجهة الأخرى.',
      );
      return;
    }
    final double sysTotal = selectedSystem.fold<double>(
      0,
      (double s, ReconciliationSystemTransaction t) => s + _systemRemaining(t),
    );
    final double stmtTotal = selectedStatement.fold<double>(
      0,
      (double s, ReconciliationStatementLine l) =>
          s + _amount(l.remainingAmount),
    );
    if ((sysTotal - stmtTotal).abs() > 0.0001) {
      setState(
        () => _matchError =
            'إجمالي الحركات المحددة (${sysTotal.toStringAsFixed(2)}) لا يساوي إجمالي أسطر الكشف المحددة (${stmtTotal.toStringAsFixed(2)}).',
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      if (selectedStatement.length > 1) {
        ...
        for (final ReconciliationStatementLine line in selectedStatement) {
          await _repository.matchReconciliation(widget.reconciliationId, <
            String,
            dynamic
          >{
            ...
            'idempotencyKey':
                'reconciliation-match-${widget.reconciliationId}-${sys.journalEntryId}-${line.id}-$stamp',
          });
        }
      } else {
  ...
  Future<void> _acceptSuggestion(
    int statementLineId,
    ReconciliationSystemTransaction candidate,
  ) async {
    try {
      await _repository.matchReconciliation(widget.reconciliationId, <
        String,
      ...
      >{
        ...
        'idempotencyKey':
            'reconciliation-suggestion-${widget.reconciliationId}-${candidate.journalEntryId}-$statementLineId',
      });
      await _load();
    } catch (error) {
  ...
  Future<void> _unmatch(ReconciliationMatchRecord match) async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (BuildContext dialog) => AlertDialog(
        ...
        content: const Text(
          'سيتم فصل هذه المطابقة وإعادة المبلغ إلى غير المطابق. هل تريد المتابعة؟',
        ),
        actions: <Widget>[
          TextButton(
    ...
    try {
      await _repository.unmatchReconciliation(
  ...
  Future<void> _deleteLine(ReconciliationStatementLine line) async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (BuildContext dialog) => AlertDialog(
        ...
        content: Text(
          'سيتم حذف السطر «${line.description}». هل تريد المتابعة؟',
        ),
        actions: <Widget>[
          TextButton(
            onPressed: () => Navigator.pop(dialog, false),
    ...
    if (confirmed != true) return;
    try {
      await _repository.deleteReconciliationStatementLine(
        widget.reconciliationId,
        line.id,
      );
      await _load();
  ...
  Future<void> _openAddLineDialog() async {
    ...
    try {
      await _repository.addReconciliationStatementLine(
        widget.reconciliationId,
        result,
      );
      await _load();
  ...
  Future<void> _openBalanceDialog({required bool actualCash}) async {
    final String? value = await showDialog<String>(
      context: context,
      builder: (BuildContext dialog) => _BalanceDialog(
        label: actualCash ? 'النقد الفعلي' : 'الرصيد الختامي الخارجي',
      ),
    );
    if (value == null) return;
    try {
      await _repository.updateReconciliation(
        widget.reconciliationId,
        <String, dynamic>{
  ...
  Future<void> _complete() async {
    final bool? confirmed = await showDialog<bool>(
      ...
      builder: (BuildContext dialog) => AlertDialog(
        ...
        content: const Text(
          'بعد الإنهاء تصبح التسوية للقراءة فقط ولا يمكن التراجع عن هذا الإجراء.',
        ),
        actions: <Widget>[
          TextButton(
            onPressed: () => Navigator.pop(dialog, false),
      ...
      pageBuilder: (BuildContext dialogContext, _, _) => Align(
        ...
        child: FinanceJournalDrawer(
          child: FinanceJournalDrawerBody(
            loader: () => _repository.getFinanceMap(
              'finance/transactions/$journalEntryId',
            ),
          ...
          if (!ready) ...<Widget>[
            ...
            ...session.blockingReasons.map(
              (String code) => Padding(
                ...
                child: Text(
                  '• ${_blockerLabels[code] ?? code}',
                  style: FinanceText.small.copyWith(color: colors.foreground),
                ),
              ),
            ),
          ],
  ...
  Widget build(BuildContext context) {
    if (transactions.isEmpty) {
      return const FinanceEmptyState(
        message: 'لا توجد حركات نقدية ضمن فترة التسوية',
      );
    }
    return FinanceTable(
      minWidth: 900,
      ...
      onRowTap: (int index) =>
          onOpenJournal(transactions[index].journalEntryId),
      rows: transactions
          .map(
...
class _BankCardWorkspace extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final double sysTotal = transactions
        .where(
          (ReconciliationSystemTransaction t) =>
              selectedSystem.contains(t.journalEntryId),
        )
        .fold<double>(
          0,
          (double s, ReconciliationSystemTransaction t) =>
              s + _amt(t.amount) - _amt(t.matchedAmount),
        );
    final double stmtTotal = session.statementLines
        .where(
          (ReconciliationStatementLine l) => selectedStatement.contains(l.id),
        )
        .fold<double>(
          0,
          (double s, ReconciliationStatementLine l) =>
              s + _amt(l.remainingAmount),
        );
    final double diff = sysTotal - stmtTotal;
    final bool canMatch =
        !readOnly &&
        selectedSystem.isNotEmpty &&
        selectedStatement.isNotEmpty &&
        diff.abs() < 0.0001 &&
        !matching;
        ...
        if (!readOnly) ...<Widget>[
          ...
          Container(
            ...
            child: Wrap(
              ...
              children: <Widget>[
                Text(
                  'إجمالي النظام: ${sysTotal.toStringAsFixed(2)}',
                  style: FinanceText.body,
                ),
                Text(
                  'إجمالي الكشف: ${stmtTotal.toStringAsFixed(2)}',
                  style: FinanceText.body,
                ),
                Text(
                      ...
                      (ReconciliationSystemTransaction candidate) => Padding(
                        ...
                        child: Row(
                          children: <Widget>[
                            Expanded(
                              child: Text(
                                '${candidate.reference} — ${candidate.amount}',
                                style: FinanceText.body,
                              ),
                            ),
                            TextButton(
          ...
          FinanceTable(
            ...
            rows: session.matches.map((ReconciliationMatchRecord match) {
              final ReconciliationStatementLine? line = session.statementLines
                  .where(
                    (ReconciliationStatementLine l) =>
                        l.id == match.statementLineId,
                  )
                  .firstOrNull;
              return <Widget>[
                FinanceReference(reference: match.journalReference),
                Text(
                  line?.reference ?? '#${match.statementLineId}',
                  style: FinanceText.body,
                ),
        ...
        if (transactions.isEmpty)
          const Padding(
            ...
            child: FinanceEmptyState(
              message: 'لا توجد حركات نظام ضمن فترة التسوية',
            ),
          )
        else
          ...transactions.map((ReconciliationSystemTransaction t) {
            ...
            return Container(
              ...
              padding: const EdgeInsets.symmetric(
                ...
                vertical: FinanceSpace.xs,
              ),
              decoration: BoxDecoration(
                color: isSelected
                    ? FinanceColors.workspace
                    : Colors.transparent,
              ...
              child: Row(
                children: <Widget>[
                  if (!readOnly && !matched)
                    Checkbox(
                      value: isSelected,
                      onChanged: (_) => onToggle(t.journalEntryId),
                    )
                  else if (matched)
                    const Padding(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text(
                          '${line.reference} · ${line.transactionDate}',
                          style: FinanceText.small.copyWith(
                            color: FinanceColors.supporting,
                          ),
                        ),
...
class _AddStatementLineDialog extends StatefulWidget {
  ...
  @override
  State<_AddStatementLineDialog> createState() =>
      _AddStatementLineDialogState();
}
