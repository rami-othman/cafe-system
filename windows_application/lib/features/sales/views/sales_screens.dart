...
class _SalesInvoiceFormScreenState extends State<SalesInvoiceFormScreen> { final form = GlobalKey<FormState>(); final reference = TextEditingController(); final notes = TextEditingController(); final invoiceDiscount = TextEditingController(); final manualAdjustment = TextEditingController(); String? invoiceDiscountType; DateTime date = DateTime.now(); int? customerId; int? branchId; List<SalesCustomer> customers = const []; List<SalesProduct> products = const []; List<Branch> branches = const []; final List<_EditLine> lines = <_EditLine>[]; final List<_EditCharge> charges = <_EditCharge>[]; bool saving = false; SalesCubit get cubit => context.read<SalesCubit>();
...
class _EditLine { _EditLine({required this.productId, this.variantId, String quantity = '1', String unitPrice = '0', this.discountType, String? discountValue}) : quantity = TextEditingController(text: quantity), unitPrice = TextEditingController(text: unitPrice), discountValue = TextEditingController(text: discountValue ?? ''); int productId; int? variantId; String? discountType; final TextEditingController quantity; final TextEditingController unitPrice; final TextEditingController discountValue; void dispose() { quantity.dispose(); unitPrice.dispose(); discountValue.dispose(); } }
class _EditCharge { _EditCharge({String? name, String? amount, this.taxable = true}) : name = TextEditingController(text: name ?? ''), amount = TextEditingController(text: amount ?? ''); final TextEditingController name; final TextEditingController amount; bool taxable; void dispose() { name.dispose(); amount.dispose(); } }
