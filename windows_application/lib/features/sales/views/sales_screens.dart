...
class _SalesCenterScreenState extends State<SalesCenterScreen> {
  ...
  @override Widget build(BuildContext context) => FinanceShell(title: 'المبيعات', subtitle: 'فواتير المبيعات والذمم والتحصيلات.', actions: <Widget>[OutlinedButton.icon(onPressed: () => context.go(AppRoutes.financeCustomersReceivables), icon: const Icon(Icons.groups_2_outlined), label: const Text('العملاء والمستحقات')), const SizedBox(width: 8), OutlinedButton.icon(onPressed: () => context.go(AppRoutes.financeSalesCreditNotes), icon: const Icon(Icons.assignment_return_outlined), label: const Text('الإشعارات الدائنة / المرتجعات')), const SizedBox(width: 8), ElevatedButton.icon(onPressed: () => context.go(AppRoutes.financeSalesNew), icon: const Icon(Icons.add), label: const Text('فاتورة مبيع'))], child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
    if (page?.financialSummary != null) ...<Widget>[
      Text('هذا الشهر (${page!.financialSummary!.periodFrom} — ${page!.financialSummary!.periodTo})', style: FinanceText.small),
      const SizedBox(height: 8),
      Row(children: <Widget>[
        Expanded(child: _kpi('صافي المبيعات', page!.financialSummary!.netSales)),
        const SizedBox(width: 12),
        Expanded(child: _kpi('عدد الفواتير المُرحّلة', '${page!.financialSummary!.postedInvoicesCount}')),
        const SizedBox(width: 12),
        Expanded(child: _kpi('الذمم المستحقة', page!.financialSummary!.outstandingAr)),
        const SizedBox(width: 12),
        Expanded(child: _kpi('المُحصّل', page!.financialSummary!.collectedTotal)),
        const SizedBox(width: 12),
        Expanded(child: _kpi('الإشعارات الدائنة', page!.financialSummary!.creditNotesTotal)),
      ]),
  ...
  Widget _body() { if (page == null && loading) return const FinanceLoadingState(label: 'جارٍ تحميل فواتير المبيعات…'); if (page == null) return FinanceErrorState(message: 'تعذر تحميل فواتير المبيعات.', onRetry: load); if (page!.items.isEmpty) return const FinanceEmptyState(message: 'لا توجد فواتير مبيعات مطابقة.'); return Opacity(opacity: loading ? .55 : 1, child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
    FinanceTable(minWidth: 1180, onRowTap: (int index) => context.go('/finance/sales/${page!.items[index].id}'), headers: const <String>['رقم الفاتورة', 'التاريخ', 'العميل', 'الفرع', 'الإجمالي', 'المدفوع', 'المتبقي', 'حالة الدفع', 'حالة المستند', 'الإجراءات'], rows: page!.items.map((i) => <Widget>[
      Text(i.invoiceNumber, style: FinanceText.body.copyWith(fontWeight: FontWeight.w700)),
      ...
      i.paidAmount == null ? Text('—', style: FinanceText.small) : FinanceAmount(value: i.paidAmount!),
      i.remainingAmount == null ? Text('—', style: FinanceText.small) : FinanceAmount(value: i.remainingAmount!, color: i.isOverdue ? FinanceColors.danger : null),
      _paymentStatusChip(i.paymentStatus),
      _status(i.status),
      i.canRegisterPayment ? TextButton(onPressed: () => _registerPayment(context, i), child: const Text('تسجيل دفعة')) : Text('—', style: FinanceText.small),
    ]).toList(growable: false)),
    FinancePagination(meta: FinancePageMeta(currentPage: page!.currentPage, perPage: 25, total: page!.total, lastPage: page!.lastPage), onPageChanged: (int value) { currentPage = value; load(); }),
  ]))); }
  Widget _status(String s) => FinanceStatusBadgeCustom(label: s == 'draft' ? 'مسودة' : (s == 'posted' ? 'مُرحّلة' : 'ملغاة'), tone: s == 'posted' ? FinanceTone.success : (s == 'cancelled' ? FinanceTone.danger : FinanceTone.warning));
...
Widget _paymentStatusChip(String? status) { if (status == null || status == 'not_applicable') return Text('—', style: FinanceText.small); final tones = <String, FinanceTone>{'unpaid': FinanceTone.neutral, 'partial': FinanceTone.warning, 'paid': FinanceTone.success, 'overdue': FinanceTone.danger}; final labels = <String, String>{'unpaid': 'غير مدفوعة', 'partial': 'مدفوعة جزئياً', 'paid': 'مدفوعة', 'overdue': 'متأخرة'}; return FinanceStatusBadgeCustom(label: labels[status] ?? status, tone: tones[status] ?? FinanceTone.neutral); }
...
class _SalesInvoiceDetailScreenState extends State<SalesInvoiceDetailScreen> { SalesInvoice? invoice; Object? error; SalesCubit get cubit => context.read<SalesCubit>(); @override void initState() { super.initState(); load(); } Future<void> load() async { try { final r = await cubit.repository.invoice(widget.id); if (mounted) setState(() { invoice = r; error = null; }); } catch (e) { if (mounted) setState(() => error = e); } }
 @override Widget build(BuildContext context) { if (invoice == null) return FinanceShell(title: 'تفاصيل فاتورة المبيع', child: error == null ? const FinanceLoadingState(label: 'جارٍ التحميل…') : FinanceErrorState(message: 'تعذر تحميل الفاتورة.', onRetry: load)); final i = invoice!; final bool posted = i.status == 'posted'; return FinanceShell(title: i.invoiceNumber, subtitle: posted ? 'الحالة المحاسبية: مُرحّل • لا يوجد أثر نقدي عند الترحيل.' : 'الحالة المحاسبية: غير مُرحّل • حالة المخزون: لم يتم تنفيذ الاستهلاك بعد', actions: <Widget>[if (i.canPost) ElevatedButton.icon(onPressed: () => _confirmPost(i), icon: const Icon(Icons.post_add), label: const Text('ترحيل الفاتورة')), if (i.canPost) OutlinedButton.icon(onPressed: () => _postAndCollect(i), icon: const Icon(Icons.payments_outlined), label: const Text('ترحيل وتسجيل دفعة')), if (i.canRegisterPayment) ElevatedButton.icon(onPressed: () async { final ok = await CustomerPaymentDialog.show(context, salesRepository: cubit.repository, financeSetupRepository: context.read<FinanceSetupCubit>().repository, customerId: i.customerId, customerName: i.customerName, branchId: i.branchId, preselectedInvoiceId: i.id); if (ok == true) load(); }, icon: const Icon(Icons.add_card), label: const Text('+ تسجيل دفعة')), if (i.canCreateCreditNote) OutlinedButton.icon(onPressed: () => context.go(Uri(path: AppRoutes.financeSalesCreditNoteNew, queryParameters: <String, String>{'invoiceId': '${i.id}', 'customerName': i.customerName}).toString()), icon: const Icon(Icons.assignment_return_outlined), label: const Text('+ إنشاء مرتجع / إشعار دائن')), if (i.canEdit) OutlinedButton.icon(onPressed: () => context.go('/finance/sales/${i.id}/edit'), icon: const Icon(Icons.edit), label: const Text('تعديل')), if (i.canCancel) TextButton(onPressed: () async { await cubit.repository.cancel(i.id); if (mounted) load(); }, child: const Text('إلغاء المسودة'))], child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Wrap(spacing: 28, runSpacing: 10, children: <Widget>[_field('العميل', i.customerName), _field('الفرع', i.branchName), _field('التاريخ', i.invoiceDate), _field('الاستحقاق', i.dueDate ?? '—'), _field('المرجع', i.reference ?? '—'), if (posted) _field('مرجع القيد', i.journalReference ?? '—')]),
   if (posted) ...<Widget>[const SizedBox(height: 20), Wrap(spacing: 28, runSpacing: 10, children: <Widget>[_field('إجمالي الفاتورة', i.total), _field('المدفوع', i.paidAmount ?? '0.00'), _field('المُعاد/المُخصوم', i.creditedAmount ?? '0.00'), _field('المتبقي', i.remainingAmount ?? i.total), Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Text('حالة الدفع', style: FinanceText.small), const SizedBox(height: 4), _paymentStatusChip(i.paymentStatus)]), Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Text('حالة الإشعارات الدائنة', style: FinanceText.small), const SizedBox(height: 4), _creditStatusChip(i.creditStatus)])])],
   const SizedBox(height: 24), SingleChildScrollView(scrollDirection: Axis.horizontal, child: DataTable(columns: const <DataColumn>[DataColumn(label: Text('المنتج')), DataColumn(label: Text('النوع')), DataColumn(label: Text('الكمية')), DataColumn(label: Text('سعر الوحدة')), DataColumn(label: Text('الخصم')), DataColumn(label: Text('الإجمالي قبل الخصم')), DataColumn(label: Text('الضريبة')), DataColumn(label: Text('الإجمالي'))], rows: i.lines.map((l) => DataRow(cells: <DataCell>[DataCell(Text(l.productName)), DataCell(Text(l.unitCode ?? l.variantName ?? '—')), DataCell(Text(l.quantity)), DataCell(Text(l.unitPrice)), DataCell(Text(l.discountAmount ?? '0.00')), DataCell(Text(l.lineSubtotal ?? '0.00')), DataCell(Text(l.taxTotal)), DataCell(Text(l.total))])).toList())), const SizedBox(height: 16), Align(alignment: AlignmentDirectional.centerEnd, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[_field('إجمالي البنود', i.grossSubtotal ?? i.subtotal), _field('خصومات البنود', i.lineDiscountTotal ?? '0.00'), _field('خصم الفاتورة', i.invoiceDiscountTotal ?? '0.00'), _field('رسوم إضافية', i.additionalChargesTotal ?? '0.00'), _field('الإجمالي قبل الضريبة', i.subtotal), _field('الضريبة', i.taxTotal), _field('الإجمالي النهائي', i.total)])),
   ...
   if (posted) ...<Widget>[const SizedBox(height: 28), Text('سجل الإشعارات الدائنة / المرتجعات', style: FinanceText.title), const SizedBox(height: 8), i.creditNotes.isEmpty ? const Padding(padding: EdgeInsets.symmetric(vertical: 12), child: Text('لا توجد إشعارات دائنة على هذه الفاتورة بعد.')) : DataTable(columns: const <DataColumn>[DataColumn(label: Text('رقم الإشعار')), DataColumn(label: Text('التاريخ')), DataColumn(label: Text('السبب')), DataColumn(label: Text('الإجمالي')), DataColumn(label: Text('خصم من الذمم')), DataColumn(label: Text('رصيد للعميل'))], rows: i.creditNotes.map((c) => DataRow(onSelectChanged: (_) => context.go('${AppRoutes.financeSalesCreditNotes}/${c.id}'), cells: <DataCell>[DataCell(Text(c.creditNoteNumber)), DataCell(Text(c.creditDate)), DataCell(Text(c.reason ?? '—')), DataCell(Text(c.total)), DataCell(Text(c.arReductionAmount)), DataCell(Text(c.customerCreditAmount))])).toList())]
 ]))); }
...
class _SalesInvoiceFormScreenState extends State<SalesInvoiceFormScreen> { final form = GlobalKey<FormState>(); final reference = TextEditingController(); final notes = TextEditingController(); final invoiceDiscount = TextEditingController(); final manualAdjustment = TextEditingController(); String? invoiceDiscountType; DateTime date = DateTime.now(); DateTime? dueDate; int? customerId; int? branchId; List<SalesCustomer> customers = const []; List<SalesProduct> products = const []; List<SalesMaterial> materials = const []; double taxRate = 0; List<Branch> branches = const []; final List<_EditLine> lines = <_EditLine>[]; final List<_EditCharge> charges = <_EditCharge>[]; int? savedId; late final String createKey = 'sales-${DateTime.now().microsecondsSinceEpoch}-${Random().nextInt(1 << 20)}'; bool saving = false; String? catalogError; SalesCubit get cubit => context.read<SalesCubit>();
 @override void initState() { super.initState(); bootstrap(); } @override void dispose() { reference.dispose(); notes.dispose(); invoiceDiscount.dispose(); manualAdjustment.dispose(); for (final l in lines) { l.dispose(); } for (final c in charges) { c.dispose(); } super.dispose(); }
  Future<T?> _loadOptional<T>(Future<T> request, String label, List<String> errors) async {
    try { return await request; } catch (error) { errors.add('$label: $error'); return null; }
  }
  Future<void> bootstrap() async {
    ...
    final values = await Future.wait<dynamic>(<Future<dynamic>>[
      ...
      _loadOptional(context.read<FinanceSetupCubit>().repository.getBranches(), 'الفروع', errors),
    ...
    setState(() {
      ...
      if (widget.id == null && branchId == null && branches.isNotEmpty) branchId = branches.first.id;
    });
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        customerId = i.customerId; branchId = i.branchId;
        reference.text = i.reference ?? ''; notes.text = i.notes ?? '';
        ...
        lines.addAll(i.lines.map((l) => _EditLine(productId: l.inventoryItemId == null ? l.productId : null, inventoryItemId: l.inventoryItemId, unitCode: l.unitCode, variantId: l.variantId, isMaterial: l.inventoryItemId != null, quantity: l.quantity, unitPrice: l.unitPrice, defaultPrice: l.baseUnitPrice ?? l.unitPrice, discountType: l.discountType, discountValue: l.discountValue, materialOverridesTouched: l.materialOverrides.isNotEmpty, materialOverrides: l.materialOverrides.map((o) => _MaterialOverrideRow(materialId: o.inventoryItemId, materialName: o.materialName, quantity: o.quantity, unitCode: o.unitCode)).toList())));
        charges.addAll(i.charges.map((c) => _EditCharge(name: c.name, amount: c.amount)));
      });
    } catch (error) {
  ...
  void _addLine() {
    if (products.isEmpty && materials.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('لا توجد منتجات أو مواد خام متاحة للإضافة.')));
      return;
    }
    setState(() => lines.add(_EditLine(unitPrice: '0.00', defaultPrice: '0.00')));
  }
  Future<void> save({bool postAfterSave = false}) async { if (!(form.currentState?.validate() ?? false) || customerId == null || branchId == null || lines.isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اختر العميل والفرع وأضف بنداً واحداً على الأقل.'))); return; } if (lines.any((l) => l.productId == null && l.inventoryItemId == null)) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اختر منتجاً أو مادة لكل بند من بنود الفاتورة.'))); return; } if (lines.any((l) => l.materialOverridesTouched && l.materialOverrides.any((o) => o.materialId == null))) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اختر مادة لكل مكوّن في قائمة مكونات الوصفة، أو احذف السطر الفارغ.'))); return; } setState(() => saving = true); try { final due = dueDate ?? date.add(Duration(days: customers.where((c) => c.id == customerId).firstOrNull?.creditTermsDays ?? 0)); final r = await cubit.repository.save(<String, dynamic>{'branchId': branchId, 'customerId': customerId, 'invoiceDate': _date(date), 'dueDate': _date(due), 'reference': reference.text.trim().isEmpty ? null : reference.text.trim(), 'notes': notes.text.trim().isEmpty ? null : notes.text.trim(), if (widget.id == null && savedId == null) 'idempotencyKey': createKey, 'invoiceDiscountType': invoiceDiscountType, 'invoiceDiscountValue': invoiceDiscountType == null ? null : (invoiceDiscount.text.trim().isEmpty ? '0' : invoiceDiscount.text.trim()), 'charges': charges.map((c) => <String, dynamic>{'name': c.name.text.trim(), 'amount': c.amount.text.trim(), 'taxable': true}).toList(), 'lines': lines.map((l) => <String, dynamic>{if (l.productId != null) 'productId': l.productId, if (l.inventoryItemId != null) 'inventoryItemId': l.inventoryItemId, if (l.inventoryItemId != null) 'unitCode': l.unitCode, if (l.variantId != null) 'variantId': l.variantId, 'quantity': l.quantity.text.trim(), if (l.inventoryItemId != null || l.unitPrice.text.trim() != l.defaultPrice) 'unitPrice': l.unitPrice.text.trim(), 'discountType': l.discountType, 'discountValue': l.discountType == null ? null : (l.discountValue.text.trim().isEmpty ? '0' : l.discountValue.text.trim()), if (l.materialOverridesTouched) 'materialOverrides': l.materialOverrides.map((o) => <String, dynamic>{'inventoryItemId': o.materialId, 'quantity': o.quantity.text.trim(), 'unitCode': o.unitCode.text.trim()}).toList()}).toList()}, id: savedId ?? widget.id); savedId = r.id; if (postAfterSave) { await cubit.repository.post(r.id, 'sales-post-${r.id}-${DateTime.now().microsecondsSinceEpoch}'); } if (mounted) context.go('/finance/sales/${r.id}'); } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر حفظ أو ترحيل الفاتورة: $e'))); } finally { if (mounted) setState(() => saving = false); } }
  double _number(String value) => double.tryParse(value.trim()) ?? 0;
  SalesDraftPreview get _preview => SalesDraftPreview(
    lines: lines.map((line) => SalesDraftLineInput(quantity: _number(line.quantity.text), unitPrice: _number(line.unitPrice.text), discountType: line.discountType, discountValue: _number(line.discountValue.text))).toList(growable: false),
    invoiceDiscountType: invoiceDiscountType,
    invoiceDiscountValue: _number(invoiceDiscount.text),
    charges: charges.fold(0, (sum, charge) => sum + _number(charge.amount.text)),
    taxRate: taxRate,
  );
 ...
 @override Widget build(BuildContext context) => FinanceShell(title: widget.id == null ? 'فاتورة مبيع جديدة' : 'تعديل فاتورة مبيع', subtitle: 'تحسب الشاشة معاينة فقط؛ المبلغ والضريبة النهائيان يحددهما الخادم.', actions: <Widget>[ElevatedButton.icon(onPressed: saving ? null : () => save(postAfterSave: true), style: ElevatedButton.styleFrom(backgroundColor: FinanceColors.success, foregroundColor: Colors.white), icon: const Icon(Icons.check_circle_outline), label: Text(saving ? 'جارٍ الترحيل…' : 'ترحيل فاتورة المبيعات'))], child: Form(key: form, child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[if (catalogError != null) Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Text('تعذر تحميل بعض بيانات الفاتورة: $catalogError'), TextButton(onPressed: bootstrap, child: const Text('إعادة المحاولة'))]))), Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Text('بيانات الفاتورة', style: FinanceText.title), const SizedBox(height: 10), Wrap(spacing: 16, runSpacing: 12, children: <Widget>[const SizedBox(width: 190, child: InputDecorator(decoration: InputDecoration(labelText: 'رقم الفاتورة'), child: Text('يُنشأ عند الحفظ'))), _selector<SalesCustomer>('العميل', customerId, customers, (c) => c.id, (c) => '${c.name} (${c.customerNumber})', (v) => setState(() => customerId = v)), TextButton.icon(onPressed: _newCustomer, icon: const Icon(Icons.person_add), label: const Text('عميل جديد')), _selector<Branch>('الفرع', branchId, branches, (b) => b.id, (b) => b.name, (v) => setState(() => branchId = v)), OutlinedButton.icon(onPressed: () async { final d = await showDatePicker(context: context, initialDate: date, firstDate: DateTime(2020), lastDate: DateTime(2100)); if (d != null) setState(() => date = d); }, icon: const Icon(Icons.calendar_today), label: Text("تاريخ الفاتورة: ${_date(date)}")), OutlinedButton.icon(onPressed: () async { final d = await showDatePicker(context: context, initialDate: dueDate ?? date, firstDate: DateTime(2020), lastDate: DateTime(2100)); if (d != null) setState(() => dueDate = d); }, icon: const Icon(Icons.event), label: Text("الاستحقاق: ${dueDate == null ? 'تلقائي' : _date(dueDate!)}")) ]), const SizedBox(height: 14), TextFormField(controller: reference, decoration: const InputDecoration(labelText: 'المرجع')), TextFormField(controller: notes, decoration: const InputDecoration(labelText: 'البيان / ملاحظات'))]))), const SizedBox(height: 22), Row(children: <Widget>[Text('بنود الفاتورة', style: FinanceText.title), const Spacer(), OutlinedButton.icon(onPressed: (products.isEmpty && materials.isEmpty) ? null : _addLine, icon: const Icon(Icons.add), label: const Text('إضافة بند'))]), const SizedBox(height: 8), ...lines.asMap().entries.map((e) => _lineEditor(e.key, e.value)), const SizedBox(height: 18), Wrap(spacing: 12, runSpacing: 8, children: <Widget>[SizedBox(width: 170, child: DropdownButtonFormField<String>(initialValue: invoiceDiscountType, decoration: const InputDecoration(labelText: 'نوع خصم الفاتورة'), items: const [DropdownMenuItem(value: 'percent', child: Text('نسبة %')), DropdownMenuItem(value: 'fixed', child: Text('مبلغ'))], onChanged: (v) => setState(() => invoiceDiscountType = v))), SizedBox(width: 140, child: TextFormField(controller: invoiceDiscount, keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'خصم الفاتورة')))]), const SizedBox(height: 14), Text('رسوم إضافية', style: FinanceText.title), OutlinedButton.icon(onPressed: () => setState(() => charges.add(_EditCharge())), icon: const Icon(Icons.add), label: const Text('إضافة رسوم')), ...charges.asMap().entries.map((e) => Wrap(spacing: 10, children: <Widget>[SizedBox(width: 200, child: TextFormField(controller: e.value.name, decoration: const InputDecoration(labelText: 'البيان'))), SizedBox(width: 120, child: TextFormField(controller: e.value.amount, keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'المبلغ'))), IconButton(onPressed: () => setState(() { e.value.dispose(); charges.removeAt(e.key); }), icon: const Icon(Icons.delete_outline))])), Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
Text('ملخص الفاتورة', style: FinanceText.title), const Divider(),
...
const Divider(), _summaryRow('الإجمالي النهائي', _totalPreview),
const Text('المعاينة تقريبية؛ الخادم يحسب الإجمالي النهائي عند الترحيل.', style: FinanceText.small),
]))),if (lines.isEmpty) const Padding(padding: EdgeInsets.all(16), child: Text('أضف منتجات من الكتالوج الحالي.')), const SizedBox(height: 16), TextButton.icon(onPressed: saving ? null : () => save(), icon: const Icon(Icons.save_outlined), label: const Text('حفظ كمسودة'))]))));
 Widget _summaryRow(String title, double value) => Padding(padding: const EdgeInsets.symmetric(vertical: 3), child: Row(children: <Widget>[Expanded(child: Text(title)), Text(value.toStringAsFixed(2))]));
 Widget _selector<T>(String label, int? value, List<T> items, int Function(T) id, String Function(T) name, ValueChanged<int?> onChanged) => SizedBox(width: 260, child: DropdownButtonFormField<int>(initialValue: value, isExpanded: true, decoration: InputDecoration(labelText: label), items: items.map((x) => DropdownMenuItem(value: id(x), child: Text(name(x), overflow: TextOverflow.ellipsis))).toList(), onChanged: onChanged, validator: (v) => v == null ? 'مطلوب' : null));
  List<_CatalogEntry> get _catalogEntries => <_CatalogEntry>[...products.map(_CatalogEntry.product), ...materials.map(_CatalogEntry.material)];
  ...
  Future<void> _loadDefaultRecipe(_EditLine line, int variantId) async {
    try {
      ...
      setState(() {
        for (final o in line.materialOverrides) { o.dispose(); }
        line.materialOverrides = components.map((c) => _MaterialOverrideRow(materialId: c.materialId, materialName: materials.where((m) => m.id == c.materialId).firstOrNull?.name, quantity: c.quantity, unitCode: c.unitCode)).toList();
      });
    } catch (_) {
      // Informational display only — save-time behaviour is unaffected since nothing was touched.
    }
  }
  Widget _lineEditor(int index, _EditLine line) { final product = products.where((p) => p.id == line.productId).firstOrNull; final variants = product?.variants ?? const <SalesVariant>[]; final material = materials.where((m) => m.id == line.inventoryItemId).firstOrNull; final _CatalogEntry? selectedEntry = line.isMaterial ? (material == null ? null : _CatalogEntry.material(material)) : (product == null ? null : _CatalogEntry.product(product)); return Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Wrap(spacing: 12, runSpacing: 10, crossAxisAlignment: WrapCrossAlignment.center, children: <Widget>[
    SizedBox(width: 260, child: SearchableSelectField<_CatalogEntry>(key: ValueKey('sales-item-$index-${line.isMaterial}-${line.isMaterial ? line.inventoryItemId : line.productId}'), label: 'الصنف', items: _catalogEntries, selected: selectedEntry, itemLabel: (e) => e.name, itemSubtitle: (e) => e.subtitle, onSelected: (entry) {
      setState(() {
        if (entry == null) { line.productId = null; line.inventoryItemId = null; return; }
        if (entry.isMaterial) { final m = entry.material!; line.isMaterial = true; line.inventoryItemId = m.id; line.productId = null; line.unitCode = m.baseUnit; return; }
        final p = entry.product!; line.isMaterial = false; line.productId = p.id; line.inventoryItemId = null; final selected = p.variants.where((x) => x.isDefault).firstOrNull; line.variantId = selected?.id; line.defaultPrice = selected?.salePrice ?? p.salePrice; line.unitPrice.text = line.defaultPrice;
      });
      if (entry != null && !entry.isMaterial && line.variantId != null) _loadDefaultRecipe(line, line.variantId!);
    })),
    if (line.isMaterial)
      SizedBox(width: 130, child: DropdownButtonFormField<String>(key: ValueKey('sales-unit-$index-${line.inventoryItemId}-${line.unitCode}'), initialValue: line.unitCode, isExpanded: true, decoration: const InputDecoration(labelText: 'الوحدة'), items: (material?.units ?? const <String>[]).map((u) => DropdownMenuItem(value: u, child: Text(u))).toList(), onChanged: (v) => setState(() => line.unitCode = v)))
    else if (variants.isNotEmpty)
      SizedBox(width: 160, child: DropdownButtonFormField<int>(key: ValueKey('sales-variant-$index-${line.variantId}'), initialValue: line.variantId, isExpanded: true, decoration: const InputDecoration(labelText: 'النوع'), items: variants.map((v) => DropdownMenuItem(value: v.id, child: Text(v.name, overflow: TextOverflow.ellipsis))).toList(), onChanged: (v) { setState(() { line.variantId = v; line.defaultPrice = variants.where((x) => x.id == v).firstOrNull?.salePrice ?? product?.salePrice ?? '0.00'; line.unitPrice.text = line.defaultPrice; }); if (v != null) _loadDefaultRecipe(line, v); })),
    SizedBox(width: 100, child: TextFormField(key: Key('sales-line-quantity-$index'), controller: line.quantity, decoration: const InputDecoration(labelText: 'الكمية'), keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), validator: (v) => (double.tryParse(v ?? '') ?? 0) <= 0 ? 'غير صالح' : null)),
    SizedBox(width: 125, child: TextFormField(key: Key('sales-line-price-$index'), controller: line.unitPrice, decoration: const InputDecoration(labelText: 'سعر الوحدة'), keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), validator: (v) => (double.tryParse(v ?? '') ?? -1) <= (line.inventoryItemId == null ? -0.001 : 0) ? 'غير صالح' : null)),
    SizedBox(width: 125, child: DropdownButtonFormField<String>(initialValue: line.discountType, decoration: const InputDecoration(labelText: 'نوع الخصم'), items: const [DropdownMenuItem(value: 'percent', child: Text('نسبة %')), DropdownMenuItem(value: 'fixed', child: Text('مبلغ'))], onChanged: (v) => setState(() => line.discountType = v))),
    SizedBox(width: 100, child: TextFormField(controller: line.discountValue, decoration: const InputDecoration(labelText: 'الخصم'), keyboardType: TextInputType.number, onChanged: (_) => setState(() {}))),
    SizedBox(width: 100, child: InputDecorator(decoration: const InputDecoration(labelText: 'الضريبة'), child: Text('${(taxRate * 100).toStringAsFixed(2)}%'))),
    SizedBox(width: 120, child: InputDecorator(decoration: const InputDecoration(labelText: 'إجمالي البند'), child: Text((_number(line.quantity.text) * _number(line.unitPrice.text)).toStringAsFixed(2)))),
    IconButton(onPressed: () => setState(() { line.dispose(); lines.removeAt(index); }), icon: const Icon(Icons.delete_outline)),
  ...
  Widget _materialOverridesSection(_EditLine line) {
    final String countLabel = line.materialOverrides.isEmpty ? 'لا توجد مكونات' : 'مكونات الوصفة (${line.materialOverrides.length})';
    return Padding(padding: const EdgeInsets.only(top: 4), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
      TextButton.icon(onPressed: () => setState(() => line.materialOverridesExpanded = !line.materialOverridesExpanded), icon: Icon(line.materialOverridesExpanded ? Icons.expand_less : Icons.expand_more, size: 18), label: Text(countLabel + (line.materialOverridesTouched ? ' — مُعدّلة لهذه الفاتورة' : ''), style: FinanceText.small)),
      if (line.materialOverridesExpanded) ...<Widget>[
        ...line.materialOverrides.asMap().entries.map((e) => _materialOverrideRow(line, e.key, e.value)),
        TextButton.icon(onPressed: () => setState(() { line.materialOverridesTouched = true; line.materialOverrides.add(_MaterialOverrideRow(quantity: '0', unitCode: '')); }), icon: const Icon(Icons.add, size: 16), label: const Text('إضافة مكوّن')),
      ],
    ]));
  }
  Widget _materialOverrideRow(_EditLine line, int index, _MaterialOverrideRow row) {
    final SalesMaterial? selected = materials.where((m) => m.id == row.materialId).firstOrNull;
    return Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Wrap(spacing: 10, runSpacing: 8, crossAxisAlignment: WrapCrossAlignment.center, children: <Widget>[
      SizedBox(width: 220, child: SearchableSelectField<SalesMaterial>(key: ValueKey('override-material-$index-${row.materialId}'), label: 'المادة', items: materials, selected: selected, itemLabel: (m) => m.name, itemSubtitle: (m) => m.sku ?? '', onSelected: (m) => setState(() { line.materialOverridesTouched = true; row.materialId = m?.id; row.materialName = m?.name; if (m != null && row.unitCode.text.trim().isEmpty) row.unitCode.text = m.baseUnit; }))),
      SizedBox(width: 90, child: TextFormField(controller: row.quantity, decoration: const InputDecoration(labelText: 'الكمية'), keyboardType: TextInputType.number, onChanged: (_) => line.materialOverridesTouched = true)),
      SizedBox(width: 90, child: TextFormField(controller: row.unitCode, decoration: const InputDecoration(labelText: 'الوحدة'), onChanged: (_) => line.materialOverridesTouched = true)),
      IconButton(onPressed: () => setState(() { line.materialOverridesTouched = true; row.dispose(); line.materialOverrides.removeAt(index); }), icon: const Icon(Icons.delete_outline, size: 18)),
    ]));
  }
 Future<void> _newCustomer() async { final controller = TextEditingController(); final name = await showDialog<String>(context: context, builder: (BuildContext dialogContext) => AlertDialog(title: const Text('عميل جديد'), content: TextField(controller: controller, autofocus: true, decoration: const InputDecoration(labelText: 'الاسم')), actions: <Widget>[TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('إلغاء')), ElevatedButton(onPressed: () => Navigator.pop(dialogContext, controller.text.trim()), child: const Text('إنشاء'))])); controller.dispose(); if (name == null || name.isEmpty) return; try { final c = await cubit.repository.createCustomer(<String, dynamic>{'name': name}); if (mounted) setState(() { customers = <SalesCustomer>[...customers, c]; customerId = c.id; }); } catch (_) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تعذر إنشاء العميل.'))); } }
 String _date(DateTime d) => '${d.year.toString().padLeft(4, '0')}-${d.month.toString().padLeft(2, '0')}-${d.day.toString().padLeft(2, '0')}'; }
class _EditLine {
  _EditLine({this.productId, this.inventoryItemId, this.unitCode, this.variantId, this.isMaterial = false, String quantity = '1', required String unitPrice, required this.defaultPrice, this.discountType, String? discountValue, List<_MaterialOverrideRow>? materialOverrides, this.materialOverridesTouched = false})
      : quantity = TextEditingController(text: quantity), unitPrice = TextEditingController(text: unitPrice), discountValue = TextEditingController(text: discountValue ?? ''), materialOverrides = materialOverrides ?? <_MaterialOverrideRow>[];
  int? productId; int? inventoryItemId; String? unitCode; int? variantId; bool isMaterial; String defaultPrice; String? discountType;
  final TextEditingController quantity; final TextEditingController unitPrice; final TextEditingController discountValue;
  List<_MaterialOverrideRow> materialOverrides;
  bool materialOverridesTouched;
  bool materialOverridesExpanded = false;
  void dispose() { quantity.dispose(); unitPrice.dispose(); discountValue.dispose(); for (final o in materialOverrides) { o.dispose(); } }
}

/// One editable row of a line's recipe override — see [_EditLine.materialOverrides].
class _MaterialOverrideRow {
  _MaterialOverrideRow({this.materialId, this.materialName, required String quantity, required String unitCode}) : quantity = TextEditingController(text: quantity), unitCode = TextEditingController(text: unitCode);
  int? materialId;
  String? materialName;
  final TextEditingController quantity;
  final TextEditingController unitCode;
  void dispose() { quantity.dispose(); unitCode.dispose(); }
}
class _EditCharge { _EditCharge({String? name, String? amount}) : name = TextEditingController(text: name ?? ''), amount = TextEditingController(text: amount ?? ''); final TextEditingController name; final TextEditingController amount; void dispose() { name.dispose(); amount.dispose(); } }
...
class _CustomerReceivablesScreenState extends State<CustomerReceivablesScreen> {
  ...
  CustomerArOverviewSummary summary = CustomerArOverviewSummary.empty; CustomerArAgingTotals aging = CustomerArAgingTotals.empty;
  SalesCubit get cubit => context.read<SalesCubit>();
  @override void initState() { super.initState(); load(); }
  Future<void> load() async {
    try {
      final results = await Future.wait<dynamic>(<Future<dynamic>>[cubit.repository.receivablesOverview(), context.read<FinanceSetupCubit>().repository.getBranches(), cubit.repository.receivablesSummary(), cubit.repository.customerAging()]);
      if (mounted) setState(() { rows = results[0] as List<CustomerReceivableOverviewRow>; branches = results[1] as List<Branch>; summary = results[2] as CustomerArOverviewSummary; aging = results[3] as CustomerArAgingTotals; error = null; });
  ...
  Widget _agingStrip() => Wrap(spacing: 12, runSpacing: 12, children: <Widget>[
    ...
    _agingTile('رصيد العملاء الدائن', summary.totalCustomerCredit, Colors.blueGrey),
  ]);
