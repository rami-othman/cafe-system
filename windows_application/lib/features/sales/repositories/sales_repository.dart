...
class SalesRepository {
  ...
  /// Phase 5: the same endpoint's `summary` envelope field (total/current/overdue AR, total customer credit, invoice counts).
  Future<CustomerArOverviewSummary> receivablesSummary() async { final Map<String, dynamic> r = Map<String, dynamic>.from(await _api.getEnvelope('finance/customers-receivables') as Map); return r['summary'] is Map ? CustomerArOverviewSummary.fromJson(Map<String, dynamic>.from(r['summary'] as Map)) : CustomerArOverviewSummary.empty; }
  /// Phase 5: AR aging buckets as of today (`finance/reports/customer-aging`).
  Future<CustomerArAgingTotals> customerAging() async { final Map<String, dynamic> r = Map<String, dynamic>.from(await _api.get('finance/reports/customer-aging') as Map); return r['totals'] is Map ? CustomerArAgingTotals.fromJson(Map<String, dynamic>.from(r['totals'] as Map)) : CustomerArAgingTotals.empty; }
