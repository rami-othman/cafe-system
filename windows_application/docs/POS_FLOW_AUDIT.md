...
- Customer, type, and table changes after creation are not persisted. Table selection is only a hard-coded visual `12`; first-order creation sends the first backend table regardless of the displayed value or order type.
...
| P1-02 | P1 | Order context | Customer/type/table changes after first item are not saved; table is not selectable. | Create an order, change type/customer, inspect `GET /orders/{id}`. | `PATCH /orders/{id}` persists context and UI reflects selected table. | Cubit only emits local type/customer. UI displays hard-coded `12`; create sends first table for every order. | Existing update endpoint is never called and state has no selected table. | `pos_cubit.dart`, `pos_cart_panel.dart`; `PATCH /orders/{id}` | Add selected table state/UI and serialize context updates through backend. Do not send table for non-dine-in unless business rule says otherwise. |
