...
final class InventoryAccountingMapper
{
    ...
    public function impactForMovement(int $tenantId, object $movement): array
    {
        ...
        if (! $journal) {
            return [...$decision, 'status' => 'FAILED', 'message' => 'A required inventory accounting posting is missing.'];
        }
    ...
    private function decision(int $tenantId, object $movement, bool $throwForConfiguration): array
    {
        ...
        if ($type === 'sale_consumption') {
            return $this->nonPosting('ALREADY_HANDLED_BY_SOURCE', 'SALE_CONSUMPTION', 'Sale consumption is already included in the paid-order COGS journal.');
        }
        ...
        if (in_array($type, ['adjustment_in', 'adjustment_out'], true)) {
            return $this->configurationRequired($throwForConfiguration, 'Manual adjustment accounting treatment is not configured.');
        }
        if (! in_array($type, ['waste', 'stock_count_variance'], true)) {
            return $this->nonPosting('NOT_APPLICABLE', 'UNSUPPORTED_MOVEMENT_TYPE', 'No Finance mapping exists for this inventory movement type.');
        }

        $amountCents = Money::cents((string) $movement->total_cost, 'totalCost');
        if ($amountCents === 0) {
            return $this->nonPosting('NOT_APPLICABLE', 'ZERO_COST', 'The finalized movement has no monetary impact.');
        }
        $accounts = $this->accounts($tenantId, $throwForConfiguration);
        if ($accounts === null) {
            return $this->configurationRequired(false, 'Inventory Asset or Inventory Variance account is not configured.');
        }
    ...
    private function nonPosting(string $status, string $classification, string $message): array
    {
        return ['status' => $status, 'classification' => $classification, 'amount' => null, 'journalId' => null, 'journalReference' => null, 'message' => $message, 'lines' => []];
    }

    private function journalNumber(int $tenantId, int $id): ?string
    {
        return DB::table('journal_entries')->where('tenant_id', $tenantId)->where('id', $id)->value('entry_number');
    }
}
