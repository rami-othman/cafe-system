...
class StockCountService
{
    ...
    public function startBarCheck(Request $request, int $tenantId, int $shiftId, int $warehouseId, ?int $actorId): int
    {
        return DB::transaction(function () use ($tenantId, $shiftId, $warehouseId, $actorId): int {
            ...
            if ((int) $warehouse->branch_id !== (int) $shift->branch_id) {
                throw ValidationException::withMessages(['warehouseId' => 'The warehouse must belong to the shift branch.']);
            }
    ...
    public function upsertLine(int $tenantId, int $countId, array $data, ?int $actorId): void
    {
        DB::transaction(function () use ($tenantId, $countId, $data, $actorId): void {
            ...
            FinancialActor::assertBranchAccess($actorId, $tenantId, $warehouse->branch_id ? (int) $warehouse->branch_id : null);
            $line = DB::table('stock_count_lines')->where('tenant_id', $tenantId)->where('stock_count_id', $countId)->where('inventory_item_id', $data['itemId'])->lockForUpdate()->first();
    ...
    public function reviewLine(Request $request, int $tenantId, int $countId, int $itemId, array $data, ?int $actorId): void
    {
        DB::transaction(function () use ($request, $tenantId, $countId, $itemId, $data, $actorId): void {
            ...
            if (! $actorId) {
                throw ValidationException::withMessages(['managerReview' => 'A manager actor is required for this review.']);
            }
            FinancialActor::assertBranchAccess($actorId, $tenantId, $count->branch_id ? (int) $count->branch_id : null);
            $line = DB::table('stock_count_lines')->where('tenant_id', $tenantId)->where('stock_count_id', $countId)->where('inventory_item_id', $itemId)->lockForUpdate()->first();
            abort_unless($line, 404, 'Count line not found.');
            if ($line->variance_status !== 'needs_manager_review') {
                throw ValidationException::withMessages(['line' => 'This line does not require manager review.']);
            }
    ...
    public function transition(Request $request, int $tenantId, int $countId, string $action, ?int $actorId): void
    {
        DB::transaction(function () use ($request, $tenantId, $countId, $action, $actorId): void {
            $count = $this->find($tenantId, $countId, true);
            $warehouseBranchId = DB::table('warehouses')->where('tenant_id', $tenantId)->where('id', $count->warehouse_id)->value('branch_id');
            ...
            if ($action === 'start') {
                ...
                if (DB::table('stock_count_lines')->where('stock_count_id', $countId)->where('is_required', true)->where('is_counted', false)->exists()) {
                    throw ValidationException::withMessages(['lines' => 'Every required count line must be counted before submission.']);
                }
                if (DB::table('stock_count_lines')->where('stock_count_id', $countId)->where('is_counted', true)->whereIn('variance_status', ['needs_reason', 'needs_manager_review'])->where(fn ($q) => $q->whereNull('reason')->orWhereRaw("TRIM(reason) = ''"))->exists()) {
                    throw ValidationException::withMessages(['reason' => 'A reason is required for every variance outside tolerance.']);
                }
                $changes = ['status' => 'submitted', 'submitted_at' => now(), 'counted_by' => $actorId];
            } elseif ($action === 'approve') {
                $this->requireStatus($count, 'submitted');
                if (DB::table('stock_count_lines')->where('stock_count_id', $countId)->where('variance_status', 'needs_manager_review')->where(fn ($q) => $q->whereNull('manager_review_status')->orWhere('manager_review_status', '!=', 'approved'))->exists()) {
                    throw ValidationException::withMessages(['managerReview' => 'All manager-review variances must be approved before approval.']);
                }
                $changes = ['status' => 'approved', 'approved_at' => now(), 'approved_by' => $actorId];
            } elseif ($action === 'cancel') {
                if (in_array($count->status, ['posted', 'cancelled'], true)) {
                    throw ValidationException::withMessages(['status' => 'A posted or cancelled count cannot be cancelled.']);
                } $changes = ['status' => 'cancelled', 'cancelled_at' => now()];
            } elseif ($action === 'post') {
                $this->requireStatus($count, 'approved');
                $lines = DB::table('stock_count_lines')->where('stock_count_id', $countId)->lockForUpdate()->get();
                foreach ($lines as $line) {
                    if (! $line->is_counted) {
                        continue;
                    }
                    if ($line->variance_status === 'needs_manager_review' && $line->manager_review_status !== 'approved') {
                        throw ValidationException::withMessages(['managerReview' => 'A required manager review is still pending.']);
                    }
                ...
                throw ValidationException::withMessages(['action' => 'Unsupported count action.']);
            }
    ...
    public function find(int $tenantId, int $countId, bool $lock = false): object
    {
        ...
        abort_unless($count, 404, 'Stock count not found.');
    ...
    private function varianceStatus(object $line, int $expected, int $variance): string
    {
        ...
        $tolerance = InventoryDecimal::units($line->quantity_tolerance ?? '0');
        $percentage = ($line->tolerance_type ?? 'quantity') === 'percentage';
    ...
    private function assertItemAssigned(int $tenantId, int $warehouseId, ?object $item, string $field): void
    {
        if (! $item || ! $item->is_active || $item->deleted_at !== null) {
            throw ValidationException::withMessages([$field => 'The template contains an inactive inventory item.']);
        }
...
return $warehouse;
    ...
    private function assertEditable(object $count): void
    {
        if (! in_array($count->status, ['draft', 'in_progress'], true)) {
            throw ValidationException::withMessages(['status' => 'Count lines cannot be edited after submission.']);
        }
    }
    ...
    private function requireStatus(object $count, string $status): void
    {
        if ($count->status !== $status) {
            throw ValidationException::withMessages(['status' => 'The current count status does not permit this action.']);
        }
    }
}
