...
final class SalesInvoiceService
{
    ...
    private function pricedLines(int $tenant, array $input): array
    {
        ...
        foreach ($input as $i => $line) {
            ...
            $variant = $this->variant($tenant, $product, $line['variantId'] ?? null, $i); $qty = $this->quantityMilli($line['quantity'] ?? null, "lines.$i.quantity"); $base = Money::cents($product->price, "lines.$i.baseUnitPrice"); $unit = array_key_exists('unitPrice', $line) ? $this->nonNegativeMoney($line['unitPrice'], "lines.$i.unitPrice") : $base; $gross = intdiv(($unit * $qty) + 500, 1000);
            ...
            $out[] = ['product_id' => $product->id, 'product_variant_id' => $variant, 'product_name' => $product->name_ar ?: $product->name, 'product_sku' => $product->sku, 'quantity' => $this->milliDecimal($qty), 'base_unit_price' => Money::decimal($base), 'unit_price' => Money::decimal($unit), 'discount_type' => $type, 'discount_value' => Money::decimal($value), 'discount_amount' => Money::decimal($discount), 'discount_total' => Money::decimal($discount), 'line_subtotal' => Money::decimal($gross), 'tax_rate' => $this->rateDecimal($rate), 'subtotal' => Money::decimal($net), 'tax_total' => Money::decimal($tax), 'total' => Money::decimal($net + $tax), '_gross' => $gross, '_discount' => $discount, '_net' => $net, '_rate' => $rate];
        } return $out;
    }
    private function variant(int $tenant, object $p, mixed $input, int $i): ?int { if ($input !== null) { $v = DB::table('product_variants')->where('tenant_id', $tenant)->where('id', (int) $input)->where('product_id', $p->id)->where('is_active', true)->whereNull('deleted_at')->first(); if (! $v) throw ValidationException::withMessages(["lines.$i.variantId" => 'Select an active variant belonging to the selected product.']); return (int) $v->id; } if ((bool) $p->is_stock_tracked || (bool) $p->inventory_controlled) { $id = DB::table('product_variants')->where('tenant_id', $tenant)->where('product_id', $p->id)->where('is_active', true)->where('is_default', true)->whereNull('deleted_at')->value('id'); if (! $id) throw ValidationException::withMessages(["lines.$i.variantId" => 'Inventory-tracked products require an active sellable variant.']); return (int) $id; } return null; }
    ...
    private function replaceLines(int $tenant,int $invoice,array $lines):void { foreach($lines as $i=>$line){unset($line['_gross'],$line['_discount'],$line['_net'],$line['_rate']);DB::table('sales_invoice_lines')->insert($line+['tenant_id'=>$tenant,'sales_invoice_id'=>$invoice,'line_number'=>$i+1,'created_at'=>now(),'updated_at'=>now()]);} }
