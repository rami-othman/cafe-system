...
final class FinancialReconciliationQueryService
{
    ...
    /**
     * Journal lines carry an optional financial_location_id (added once
     * several financial_locations were allowed to share one GL account —
     * see migration 2026_09_28_000001). When the reconciliation session is
     * tied to a specific location, only lines tagged with that same location
     * count, plus untagged legacy lines predating that column, so a session
     * against one drawer never pulls in another drawer's movements on the
     * same account. Untagged legacy lines are still ambiguous by
     * construction and are accepted as a known limitation of historical
     * data rather than being rewritten.
     */
    private function eligible(int $tenant, object $session): Builder { $query = DB::table('journal_entries as entries')->join('journal_entry_lines as lines', 'lines.journal_entry_id', '=', 'entries.id')->where('entries.tenant_id', $tenant)->where('lines.tenant_id', $tenant)->where('entries.status', 'posted')->where('lines.financial_account_id', $session->financial_account_id)->whereDate('entries.entry_date', '>=', $session->date_from)->whereDate('entries.entry_date', '<=', $session->date_to); if ($session->financial_location_id !== null) $query->where(fn (Builder $q) => $q->where('lines.financial_location_id', $session->financial_location_id)->orWhereNull('lines.financial_location_id')); if ($session->branch_id !== null) $query->where('entries.branch_id', $session->branch_id); return $query->selectRaw('entries.id, entries.entry_number, entries.entry_date, entries.description, SUM(lines.debit) as debit, SUM(lines.credit) as credit')->groupBy('entries.id', 'entries.entry_number', 'entries.entry_date', 'entries.description'); }
