...
class ExpenseService
{
    ...
    public function pay(Request $request, int $tenantId, int $id, array $data, ?int $actorId): object
    {
        ...
        try { return DB::transaction(function () use ($request, $tenantId, $id, $data, $actorId, $key, $fingerprint): object {
            ...
            if ($usedKey && (int) $usedKey->id !== $id) abort(409, 'تم استخدام مفتاح العملية هذا مسبقًا لدفعة مصروف مختلفة.');
        ...
        }); } catch (QueryException $e) { $existing = DB::table('expenses')->where('tenant_id', $tenantId)->where('payment_idempotency_key', $key)->first(); if ($existing && (int) $existing->id === $id) { $this->assertPaymentFingerprint($existing, $fingerprint); return $existing; } if ($existing) abort(409, 'تم استخدام مفتاح العملية هذا مسبقًا لدفعة مصروف مختلفة.'); throw $e; }
    }
    ...
    private function assertFingerprint(object $expense, string $fingerprint): void { if (! $expense->idempotency_fingerprint || ! hash_equals($expense->idempotency_fingerprint, $fingerprint)) abort(409, 'تم استخدام مفتاح العملية هذا مسبقًا لطلب مصروف مختلف.'); }
    private function assertPaymentFingerprint(object $expense, string $fingerprint): void { if (! $expense->payment_idempotency_fingerprint || ! hash_equals($expense->payment_idempotency_fingerprint, $fingerprint)) abort(409, 'تم استخدام مفتاح العملية هذا مسبقًا لطلب دفعة مصروف مختلفة.'); }
