...
final class DailyClosingSummaryService
{
    ...
    private function aggregates(int $tenant, int $branch, $start, $end, string $day, $cashLocationIds): object
    {
        ...
        $builders = [
            ...
            'orders' => DB::table('orders')->where('tenant_id',$tenant)->where('branch_id',$branch)->where('payment_status','!=','unpaid')->where('status','!=','cancelled')->whereNull('deleted_at')->whereBetween('closed_at',[$start,$end])->selectRaw('COALESCE(SUM(subtotal + tax_total + service_total),0) orders_gross, COALESCE(SUM(discount_total),0) orders_discounts'),
