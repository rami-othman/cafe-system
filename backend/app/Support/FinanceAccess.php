...
final class FinanceAccess
{
    ...
    public static function permissionsFor(int $tenantId, string $role): array { if ($role==='owner') return self::CATALOG; return array_values(array_unique([...self::defaultPermissionsForRole($role), ...DB::table('finance_role_permissions')->where('tenant_id',$tenantId)->where('role',$role)->pluck('permission')->filter(fn($permission)=>in_array($permission,self::CATALOG,true))->values()->all()])); }
    public static function defaultPermissionsForRole(string $role): array {
        ...
        if ($role === 'cashier') return [
            'finance.vouchers.view', 'finance.vouchers.create', 'finance.vouchers.post',
            'finance.receipts.create', 'finance.payments.create',
            'finance.purchases.view', 'finance.purchases.create', 'finance.purchases.edit', 'finance.purchases.post', 'finance.purchases.receive',
            ...
            'finance.sales.view', 'finance.sales.create', 'finance.sales.edit', 'finance.sales.post',
