...
class PlatformManagementController extends Controller
{
    public function branches(Request $request): JsonResponse
    {
        ...
        if ($data['tenantId'] ?? null) $query->where('branches.tenant_id', $data['tenantId']);
        if ($data['search'] ?? null) $query->where(fn ($q) => $q->whereILike('branches.name', '%'.$data['search'].'%')->orWhereILike('tenants.name', '%'.$data['search'].'%'));
    ...
    public function updateBranch(Request $request, int $branch, PlatformAuditService $audit): JsonResponse
    {
        ...
        $before = DB::table('branches')->where('id', $branch)->whereNull('deleted_at')->first(); abort_if(! $before, 404);
        DB::table('branches')->where('id', $branch)->update(['is_active' => $data['isActive'], 'updated_at' => now()]);
        $after = DB::table('branches')->find($branch); $audit->record($request, 'branch.status_changed', 'branch', $branch, $before->tenant_id, (array) $before, (array) $after, $data['reason']);
    ...
    public function tenantUsers(Request $request): JsonResponse
    {
        ...
        if ($data['tenantId'] ?? null) $query->where('users.tenant_id', $data['tenantId']);
        if ($data['search'] ?? null) $query->where(fn ($q) => $q->whereILike('users.name', '%'.$data['search'].'%')->orWhereILike('users.email', '%'.$data['search'].'%')->orWhereILike('tenants.name', '%'.$data['search'].'%'));
    ...
    public function updateTenantUser(Request $request, int $user, PlatformAuditService $audit): JsonResponse
    {
        ...
        $before = DB::table('users')->where('id', $user)->whereNotNull('tenant_id')->whereNull('deleted_at')->first(); abort_if(! $before, 404);
        $changes = ['is_active' => $data['isActive'], 'updated_at' => now()]; if ($data['role'] ?? null) $changes['role'] = $data['role'];
        DB::table('users')->where('id', $user)->update($changes); $after = DB::table('users')->find($user); $audit->record($request, 'tenant_user.updated', 'user', $user, $before->tenant_id, (array) $before, (array) $after, $data['reason']);
        return response()->json(['data' => $after]);
    }
    ...
    public function updatePlan(Request $request, int $plan, PlatformAuditService $audit): JsonResponse
    {
        ...
        $before = DB::table('plans')->find($plan); abort_if(! $before, 404);
        ...
        $after = DB::table('plans')->find($plan); $audit->record($request, 'plan.updated', 'plan', $plan, null, (array) $before, (array) $after);
        return response()->json(['data' => $after]);
    }
    ...
    public function subscriptions(Request $request): JsonResponse
    {
        ...
        if ($data['status'] ?? null) $query->where('subscriptions.status', $data['status']);
        if ($data['endingBefore'] ?? null) $query->where('subscriptions.trial_ends_at', '<=', Carbon::parse($data['endingBefore'])->endOfDay());
    ...
    public function updateSubscription(Request $request, int $subscription, PlatformAuditService $audit): JsonResponse
    {
        ...
        $before = DB::table('subscriptions')->find($subscription); abort_if(! $before, 404);
    ...
    public function analytics(Request $request): JsonResponse
    {
        ...
        $from = Carbon::parse($data['from'] ?? now()->subDays(29))->startOfDay(); $to = Carbon::parse($data['to'] ?? now())->endOfDay();
    ...
    public function auditLogs(Request $request): JsonResponse
    {
        ...
        if ($data['action'] ?? null) $query->where('platform_audit_logs.action', $data['action']); if ($data['tenantId'] ?? null) $query->where('platform_audit_logs.tenant_id', $data['tenantId']);
        return response()->json(['data' => $query->latest('platform_audit_logs.id')->limit(100)->get()]);
    }

    public function announcements(): JsonResponse { return response()->json(['data' => DB::table('announcements')->latest()->get()]); }
    ...
    public function storeAnnouncement(Request $request, PlatformAuditService $audit): JsonResponse
    {
        ...
        $announcement = DB::table('announcements')->find($id); $audit->record($request, 'announcement.created', 'announcement', $id, null, [], (array) $announcement);
    ...
    public function updateAnnouncement(Request $request, int $announcement, PlatformAuditService $audit): JsonResponse
    {
        ...
        $before = DB::table('announcements')->find($announcement); abort_if(! $before, 404);
        ...
        $after = DB::table('announcements')->find($announcement); $audit->record($request, 'announcement.status_changed', 'announcement', $announcement, null, (array) $before, (array) $after, $data['reason'] ?? null);
        return response()->json(['data' => $after]);
    }

    public function health(): JsonResponse
    {
        $database = 'healthy'; try { DB::select('select 1'); } catch (\Throwable) { $database = 'down'; }
    ...
    public function settings(): JsonResponse { return response()->json(['data' => DB::table('platform_settings')->orderBy('key')->get()]); }

    public function updateSetting(Request $request, string $setting, PlatformAuditService $audit): JsonResponse
    {
        $data = $request->validate(['value' => ['required']]); $before = DB::table('platform_settings')->where('key', $setting)->first();
        ...
        $after = DB::table('platform_settings')->where('key', $setting)->first(); $audit->record($request, 'platform_setting.updated', 'platform_setting', null, null, (array) $before, (array) $after);
        return response()->json(['data' => $after]);
    }
