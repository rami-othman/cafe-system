...
class FinancialLocationController extends Controller
{
    ...
    private function serialize(int $tenantId, object $row): array { $today = now()->toDateString(); $locationId = (int) $row->id; $accountId = (int) $row->financial_account_id; $summary = $this->balances->summary($tenantId, $accountId, null, null, $locationId); $todaySummary = $this->balances->summary($tenantId, $accountId, $today, $today, $locationId); $external = $this->balances->summary($tenantId, $accountId, $today, $today, $locationId, true); return ['id' => $locationId, 'branchId' => $row->branch_id ? (int) $row->branch_id : null, 'branchName' => $row->branch_name, 'financialAccountId' => $accountId, 'financialAccountCode' => $row->account_code, 'financialAccountNameAr' => $row->account_name_ar, 'code' => $row->code, 'name' => $row->name, 'kind' => $row->kind, 'type' => $row->type, 'bankName' => $row->bank_name, 'maskedReference' => $row->masked_reference, 'isActive' => (bool) $row->is_active, 'balance' => $summary['balance'], 'todayIncoming' => $todaySummary['incoming'], 'todayOutgoing' => $todaySummary['outgoing'], 'todayExternalIncoming' => $external['incoming'], 'todayExternalOutgoing' => $external['outgoing']]; }
