...
class FinancialLocationController extends Controller
{
    ...
    public function update(Request $request, int $location, string $kind): JsonResponse { $data = $this->validateLocation($request, $kind, $location); $tenantId = TenantContext::id($request); $this->locations->save($request, $tenantId, $data, $location, FinancialActor::id($request, $tenantId)); return response()->json(['data' => $this->showRow($tenantId, $location)]); }
    public function status(Request $request, int $location, string $kind): JsonResponse { $data = $request->validate(['isActive' => ['required', 'boolean']]); $tenantId = TenantContext::id($request); $actorId = FinancialActor::id($request, $tenantId); $this->locationForKind($tenantId, $actorId, $kind, $location); $this->locations->setStatus($request, $tenantId, $location, $data['isActive'], $actorId); return response()->json(['data' => $this->showRow($tenantId, $location)]); }
    public function show(Request $request, int $location, string $kind): JsonResponse { $tenantId = TenantContext::id($request); $this->locationForKind($tenantId, FinancialActor::id($request, $tenantId), $kind, $location); return response()->json(['data' => $this->showRow($tenantId, $location)]); }
    public function transactions(Request $request, int $location, string $kind): JsonResponse { $tenantId = TenantContext::id($request); $row = $this->locationForKind($tenantId, FinancialActor::id($request, $tenantId), $kind, $location); $data = $request->validate(['from' => ['nullable', 'date'], 'to' => ['nullable', 'date'], 'search' => ['nullable', 'string', 'max:100']]); return response()->json(['data' => ['location' => $this->showRow($tenantId, $location), 'transactions' => $this->balances->transactions($tenantId, $row->financial_account_id, $data['from'] ?? null, $data['to'] ?? null, $data['search'] ?? null, $location)]]); }
    ...
    private function locationForKind(int $tenantId, int $actorId, string $kind, int $location): object { $row = $this->locations->find($tenantId, $location); abort_unless($row->kind === $kind, 404); FinancialActor::assertBranchAccess($actorId, $tenantId, $row->branch_id ? (int) $row->branch_id : null); return $row; }
    ...
    private function serialize(int $tenantId, object $row): array { $today = BranchLocalDate::today($row->branch_id ? (int) $row->branch_id : null); $locationId = (int) $row->id; $accountId = (int) $row->financial_account_id; $summary = $this->balances->summary($tenantId, $accountId, null, null, $locationId); $todaySummary = $this->balances->summary($tenantId, $accountId, $today, $today, $locationId); $external = $this->balances->summary($tenantId, $accountId, $today, $today, $locationId, true); return ['id' => $locationId, 'branchId' => $row->branch_id ? (int) $row->branch_id : null, 'branchName' => $row->branch_name, 'financialAccountId' => $accountId, 'financialAccountCode' => $row->account_code, 'financialAccountNameAr' => $row->account_name_ar, 'code' => $row->code, 'name' => $row->name, 'kind' => $row->kind, 'type' => $row->type, 'bankName' => $row->bank_name, 'maskedReference' => $row->masked_reference, 'isActive' => (bool) $row->is_active, 'balance' => $summary['balance'], 'todayIncoming' => $todaySummary['incoming'], 'todayOutgoing' => $todaySummary['outgoing'], 'todayExternalIncoming' => $external['incoming'], 'todayExternalOutgoing' => $external['outgoing']]; }
