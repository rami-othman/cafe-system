...
final class FinancialReportController extends Controller
{
    ...
    public function index(): JsonResponse { return response()->json(['data' => ['reports' => ['profitLoss', 'balanceSheet', 'cashFlow', 'trialBalance', 'generalLedger', 'supplierAging', 'supplierStatement', 'customerAging', 'customerStatement'], 'filters' => ['dateFrom', 'dateTo', 'asOfDate', 'branchId', 'comparison']]]); }
    ...
    public function customerAging(Request $request): JsonResponse { $data = $request->validate(['asOfDate' => ['nullable', 'date_format:Y-m-d'], 'customerId' => ['nullable', 'integer']]); $ctx = $this->context($request); return response()->json(['data' => $this->reports->customerAging($ctx, $data['asOfDate'] ?? $ctx['dateTo'], isset($data['customerId']) ? (int) $data['customerId'] : null)]); }
    public function customerStatement(Request $request): JsonResponse { $data = $request->validate(['customerId' => ['required', 'integer']]); $ctx = $this->context($request); return response()->json(['data' => $this->reports->customerStatement($ctx, (int) $data['customerId'])]); }
