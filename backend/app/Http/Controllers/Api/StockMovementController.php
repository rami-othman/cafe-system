...
class StockMovementController extends Controller
{
    ...
    public function store(StockMovementRequest $request): JsonResponse
    {
        ...
        $id = $this->movements->record($request, $tenant, $request->validated(), FinancialActor::id($request, $tenant));

        return response()->json(['data' => $this->serialize($this->find($tenant, $id))], 201);
    }
    ...
    public function show(Request $request, int $movement): JsonResponse
    {
        ...
        return response()->json(['data' => $this->serialize($this->find($tenant, $movement))]);
    }
    ...
    {
        return ['id' => (int) $row->id, 'warehouseId' => (int) $row->warehouse_id, 'warehouseName' => WarehousePresentation::displayName($row->branch_name, $row->warehouse_type), 'warehouseTypeLabel' => WarehousePresentation::typeLabel($row->warehouse_type), 'itemId' => (int) $row->inventory_item_id, 'itemNameAr' => $row->item_name_en ?: $row->sku, 'unit' => $row->unit, 'type' => $row->type, 'quantityIn' => InventoryDecimal::quantity(InventoryDecimal::units($row->quantity_in)), 'quantityOut' => InventoryDecimal::quantity(InventoryDecimal::units($row->quantity_out)), 'quantityBefore' => InventoryDecimal::quantity(InventoryDecimal::units($row->quantity_before)), 'quantityAfter' => InventoryDecimal::quantity(InventoryDecimal::units($row->quantity_after)), 'unitCost' => InventoryDecimal::unitCost(InventoryDecimal::cost($row->unit_cost)), 'totalCost' => $row->total_cost, 'reason' => $row->reason, 'referenceType' => $row->reference_type, 'referenceId' => $row->reference_id ? (int) $row->reference_id : null, 'userName' => $row->user_name, 'occurredAt' => $row->occurred_at, 'createdAt' => $row->created_at];
    }
}
