...
class ExpenseController extends Controller
{
    ...
    public function pay(Request $request, int $expense): JsonResponse { $tenant = TenantContext::id($request); $data = $request->validate(['paymentMethodId' => ['required', 'integer'], 'financialLocationId' => ['nullable', 'integer'], 'paymentDate' => ['required', 'date'], 'description' => ['nullable', 'string', 'max:1000'], 'idempotencyKey' => ['required', 'string', 'max:120']]); $this->expenses->pay($request, $tenant, $expense, $data, FinancialActor::id($request, $tenant)); return response()->json(['data' => $this->one($tenant, $expense, $request)]); }
