...
final class AccountingPeriodController extends Controller
{
    ...
    public function index(Request $request): JsonResponse { $tenant = TenantContext::id($request); $data = $request->validate(['page' => ['nullable', 'integer', 'min:1'], 'perPage' => ['nullable', 'integer', 'min:1', 'max:100']]); $permissions=array_fill_keys(FinanceAccess::capabilities($request),true); $paginator = DB::table('accounting_periods')->where('tenant_id', $tenant)->orderByDesc('start_date')->paginate((int) ($data['perPage'] ?? 10), ['*'], 'page', (int) ($data['page'] ?? 1)); return response()->json(['data' => collect($paginator->items())->map(fn ($r) => $this->serialize($r)+['allowedActions'=>$r->status==='open'&&isset($permissions['finance.periods.manage'])?['edit']:[]])->values(), 'meta' => ['currentPage' => $paginator->currentPage(), 'perPage' => $paginator->perPage(), 'total' => $paginator->total(), 'lastPage' => $paginator->lastPage()]]); }
