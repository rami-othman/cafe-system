...
final class DailyClosingController extends Controller
{
    ...
    /**
     * Every row (open or closed) is re-derived through `DailyClosingService::present()` so the
     * list's net sales, expected/actual cash, difference, and reconciliation-driven readiness are
     * the same authoritative figures as the detail workspace — never the raw (often NULL-for-open)
     * `daily_closings` snapshot columns, which are only populated on close.
     */
    public function index(Request $r): JsonResponse { [$t,$a]=$this->actor($r);$d=$r->validate(['branchId'=>['nullable','integer'],'from'=>['nullable','date'],'to'=>['nullable','date'],'status'=>['nullable','in:open,closed'],'perPage'=>['nullable','integer','min:1','max:100']]);$q=DB::table('daily_closings as c')->join('branches as b','b.id','=','c.branch_id')->where('c.tenant_id',$t)->whereIn('c.branch_id', FinancialActor::operationalBranchIds($a,$t));foreach(['branchId'=>'c.branch_id','status'=>'c.status']as$k=>$col)if(!empty($d[$k]))$q->where($col,$d[$k]);if(!empty($d['from']))$q->whereDate('c.business_date','>=',$d['from']);if(!empty($d['to']))$q->whereDate('c.business_date','<=',$d['to']);$p=$q->orderByDesc('c.business_date')->paginate($d['perPage']??25,['c.*','b.name as branch_name']);$permissions=array_fill_keys(FinanceAccess::capabilities($r),true);return response()->json(['data'=>collect($p->items())->map(function($x) use ($t,$permissions) {$view=$this->service->present($t,(int)$x->branch_id,$x->business_date,$x);return['id'=>$x->id,'reference'=>$x->reference,'businessDate'=>$x->business_date,'branch'=>['id'=>$x->branch_id,'name'=>$x->branch_name],'status'=>$x->status,'readiness'=>$view['readiness'],'warningsCount'=>count($view['warnings']),'netSales'=>$view['sales']['netSales'],'expectedCash'=>$view['cash']['expectedCash'],'actualCash'=>$view['cash']['actualCash'],'difference'=>$view['cash']['difference'],'closedBy'=>$x->closed_by,'closedAt'=>$x->closed_at,'allowedActions'=>$x->status==='open'?array_values(array_filter(['edit'=>isset($permissions['finance.daily_closing.manage'])?'edit':null,'close'=>isset($permissions['finance.daily_closing.close'])?'close':null])):[]];})->values(),'meta'=>['currentPage'=>$p->currentPage(),'perPage'=>$p->perPage(),'total'=>$p->total(),'lastPage'=>$p->lastPage()]]); }
