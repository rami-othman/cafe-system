...
final class SalesInvoicePosCrossPathCharacterizationTest extends TestCase
{
    ...
    public function test_pos_and_manual_invoice_consume_the_same_18_grams_per_cappuccino_from_the_same_branch_warehouse(): void
    {
    ...
    public function test_pos_and_manual_invoice_agree_on_empty_consumption_while_using_their_own_authorities(): void
    {
        $x = $this->scenario(true); $request = Request::create('/sales', 'POST'); $request->attributes->set('tenant_id', $x['tenant']); $request->attributes->set('auth_user', $x['user']);
        $preview = app(SalesInvoicePostingService::class)->preview($x['tenant'], $x['invoice']->id);
        $this->assertSame([], $preview['inventory']); $this->assertSame('0.00', $preview['cogs']['totalEstimated']);
        $order = DB::table('orders')->where('id', $x['order'])->first(); $pos = app(SaleConsumptionService::class)->consumeForOrder($request, $x['tenant'], $order, null, $x['owner']);
        app(SalesInvoicePostingService::class)->post($request, $x['tenant'], $x['invoice']->id, $x['owner'], ['idempotencyKey' => 'cross-path-empty']);
        $this->assertSame(0, $pos['cogsTotalCents']);
        $this->assertSame(0, DB::table('stock_movements')->where('tenant_id', $x['tenant'])->whereIn('reference_type', ['order_item', 'sales_invoice_line'])->count());
        $this->assertSame('0.00', DB::table('sales_invoice_lines')->where('sales_invoice_id', $x['invoice']->id)->value('cogs_total'));
    }

    private function scenario(bool $emptyRecipe = false): array
    {
        ...
        app(FinancialSetupService::class)->ensureForTenant($tenant, $branch, $owner); $warehouse = (int) DB::table('branches')->where('tenant_id', $tenant)->where('id', $branch)->value('pos_inventory_warehouse_id');
