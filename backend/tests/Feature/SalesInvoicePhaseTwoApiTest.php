...
class SalesInvoicePhaseTwoApiTest extends TestCase
{
    ...
    /** dueDate is pinned far in the future (relative to now(), never a fixed calendar date) so paymentStatus assertions never spuriously flip to 'overdue' as real time passes invoiceDate — this test is about posting/COGS/journals, not overdue labeling. */
    private function invoice(array $s, int $product, string $quantity): int { $customer = (int) $this->postJson('/api/v1/finance/customers', ['name' => 'Damascus Tech Company'], $s['headers'])->assertCreated()->json('data.id'); return (int) $this->postJson('/api/v1/finance/sales-invoices', ['branchId' => $s['branch'], 'customerId' => $customer, 'invoiceDate' => '2026-09-12', 'dueDate' => now()->addYear()->toDateString(), 'lines' => [['productId' => $product, 'quantity' => $quantity]]], $s['headers'])->assertCreated()->json('data.id'); }
}
