...
class InventoryCenterSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($this->items() as $position => $item) {
            ...
            // A stock count can only include stock explicitly available at its
            // warehouse. Seed the catalogue at every active location; opening
            // balances still belong only to the selected source warehouse.
            foreach ($warehouses as $availableWarehouse) {
                DB::table('inventory_item_warehouses')->updateOrInsert(
                    ['tenant_id' => $tenant, 'inventory_item_id' => $itemId, 'warehouse_id' => $availableWarehouse->id],
    ...
    private function items(): array
    {
        ...
        return array_values(array_filter(
            array_merge($items, Cafe618ProvidedMaterials::items()),
            static function (array $item) use (&$names): bool {
                $name = InventoryCatalogIdentity::normalize($item['nameAr']);
                if (isset($names[$name])) {
                    return false;
                }
