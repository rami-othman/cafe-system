...
final class Cafe618PosSalesDemoSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        try {
            foreach ($this->saleDays() as $index => $daysAgo) {
                ...
                $productId = $index % 3 === 2 ? $products['iced'] : $products['cappuccino'];
                $method = $index % 3 === 0 ? 'card' : 'cash';
                $this->sale($daysAgo, $index, $shift, $menu, $productId, $method, 1 + ($index % 2));
                if ($daysAgo > 0) {
    ...
    private function configureProductsAndRecipes(): array
    {
        ...
        foreach (['cappuccino' => 'Cappuccino', 'iced' => 'Iced Caramel Macchiato'] as $key => $name) {
            ...
            DB::table('products')->where('id', $id)->update(['inventory_controlled' => true, 'consumption_type' => 'bar', 'updated_at' => now()]);
    ...
    private function publishedSnapshot(array $products): array
    {
        ...
        $snapshotProducts = []; $placements = []; $variants = [];
        foreach ($products as $productId) {
            ...
            $placements[$productId] = $placementId; $variants[$productId] = (int) $variant->id;
        }
    ...
    private function paymentMethod(string $code): int { return (int) DB::table('payment_methods')->where('tenant_id', $this->tenantId)->where('code', $code)->where('is_active', true)->value('id'); }
    private function request(string $uri, array $data): Request { $request = Request::create($uri, 'POST', $data); $request->attributes->set('tenant_id', $this->tenantId); $request->attributes->set('auth_user', $this->owner); return $request; }
    /** @return list<int> */
    private function saleDays(): array { return [59, 56, 53, 50, 47, 44, 41, 38, 35, 32, 29, 26, 23, 20, 17, 14, 11, 8, 6, 4, 2, 0]; }
}
