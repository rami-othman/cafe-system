...
final class FinanceOperationsDemoSeeder extends Seeder
{
     ...
     */
    private function closeDate(): string
    {
    ...
    {
        DB::table('products')->updateOrInsert(['tenant_id' => $tenant, 'sku' => 'DEMO-LATTE'], ['name' => 'Demo Latte', 'description' => 'Finance operations demo drink', 'price' => '35.00', 'cost_price' => '0.00', 'is_active' => true, 'is_stock_tracked' => true, 'consumption_type' => 'bar', 'created_at' => now(), 'updated_at' => now()]);
        ...
        $recipe = VariantRecipe::firstOrCreate(['tenant_id' => $tenant, 'product_variant_id' => $variant]);
        foreach ([[$beans, '0.020', 'kg'], [$milk, '0.200', 'l']] as $number => [$item, $quantity, $unit]) {
            $recipe->components()->updateOrCreate(['inventory_item_id' => $item], ['tenant_id' => $tenant, 'quantity' => $quantity, 'unit_code' => $unit, 'sort_order' => $number]);
        }
        $menu = DB::table('menus')->where('tenant_id', $tenant)->where('name', 'Finance Demo POS')->value('id');
        if (! $menu) {
            $menu = DB::table('menus')->insertGetId(['tenant_id' => $tenant, 'name' => 'Finance Demo POS', 'status' => 'draft', 'priority' => 0, 'created_at' => now(), 'updated_at' => now()]);
        }
        ...
        if (! $version) {
            ...
            $payload = app(PublishedMenuSnapshotBuilder::class)->build($tenant, Branch::findOrFail($branch), 'pos', [(int) $menu]);
    ...
    private function saleAndRefund(int $tenant, int $owner, int $branch, int $product, int $variant, int $placement, int $version): void
    {
        ...
        $refundRequest->attributes->set('tenant_id', $tenant);
        $refundRequest->attributes->set('auth_user', $authUser);
    ...
    private function postExpense(Request $request, int $tenant, int $owner, int $branch, int $category, string $amount, string $date, string $description, string $idempotencyKey, bool $pay = true): void
    {
        ...
        if ($expense->status === 'draft') {
            $expense = $service->transition($request, $tenant, $expense->id, 'submit', [], $owner);
        }
        if ($expense->status === 'pending_approval') {
            $expense = $service->transition($request, $tenant, $expense->id, 'approve', [], $owner);
        }
        if ($pay && $expense->status === 'approved') {
            $service->pay($request, $tenant, $expense->id, ['paymentMethodId' => DB::table('payment_methods')->where('tenant_id', $tenant)->where('code', 'CASH')->value('id'), 'financialLocationId' => DB::table('financial_locations')->where('tenant_id', $tenant)->where('code', 'CASH-DRAWER')->value('id'), 'paymentDate' => $date, 'idempotencyKey' => $idempotencyKey.'-pay'], $owner);
        }
    }
    ...
    private function postedInvoice(Request $request, int $tenant, int $owner, array $data): object
    {
        ...
        if ($invoice->status === 'draft') {
            $invoice = $service->post($request, $tenant, $invoice->id, ['idempotencyKey' => $data['idempotencyKey'].'-post'], $owner);
        }
    ...
    private function payment(Request $request, int $tenant, int $owner, int $branch, int $supplier, int $invoice, string $amount, string $date, int $method, int $location, string $key): void
    {
        ...
        if (in_array($status, ['posted', 'partially_paid'], true)) {
            app(SupplierPaymentService::class)->pay($request, $tenant, ['branchId' => $branch, 'supplierId' => $supplier, 'paymentMethodId' => $method, 'financialLocationId' => $location, 'paymentDate' => $date, 'amount' => $amount, 'allocations' => [['invoiceId' => $invoice, 'amount' => $amount]], 'idempotencyKey' => $key], $owner);
        }
    }
    ...
    private function reconcileAndClose(Request $request, int $tenant, int $owner, int $branch): void
    {
        ...
        if ($session->status !== 'completed') {
            $reconciliations->complete($request, $tenant, $session->id, $owner, app(FinancialReconciliationQueryService::class));
        }
    ...
    private function closePeriod(Request $request, int $tenant, int $owner): void
    {
        ...
        if (! $period) {
            $period = $service->create($request, $tenant, ['name' => 'Finance Demo July-August 2026', 'startDate' => '2026-07-01', 'endDate' => $this->closeDate(), 'notes' => 'Closed deterministic Finance Operations demo period'], $owner);
        }
        if ($period->status === 'open') {
            $readiness = $service->readiness($tenant, $period->id);
            if (! $readiness['canClose']) {
                throw new RuntimeException('Finance demo accounting period readiness failed: '.json_encode($readiness['blockers'], JSON_THROW_ON_ERROR));
            }
